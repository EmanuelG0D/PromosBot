...
SIN_PRECIO_DE_LISTA = ("slickdeals", "promocajita", "republica", "miloderrocha", "descuentostech")
...
def evaluar(deal: Deal, stats: tuple[int, float | None, float | None],
    ...
    if deal.source in SIN_PRECIO_DE_LISTA:
        return Verdict(True, f"destacada en {deal.store}", etiquetas, confianza,
                       glitch, inmediata=urgente or bool(deal.coupons) or deal.source in {"republica", "miloderrocha", "descuentostech"})
